...
class SantanderCustomerTransactionPredictionMetrics(CompetitionMetrics):
    """Metric class for Santander Customer Transaction Prediction competition using ROC AUC."""
    def __init__(self, value: str = "target", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert first column (ID_code) to string type
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Check if first columns (ID_code) are identical
        if (submission[submission.columns[0]].values != ground_truth[ground_truth.columns[0]].values).any():
            raise InvalidSubmissionError(
                "First column values (ID_code) do not match between submission and ground truth. Please ensure the first column values are identical."
            )

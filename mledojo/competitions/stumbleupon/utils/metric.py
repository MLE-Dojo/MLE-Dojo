...
class StumbleuponMetrics(CompetitionMetrics):
    """Metric class for the StumbleUpon Evergreen Classification Challenge using AUC"""
    def __init__(self, value: str = "label", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert first column (urlid) to string type
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Check if the first column values match between submission and ground truth

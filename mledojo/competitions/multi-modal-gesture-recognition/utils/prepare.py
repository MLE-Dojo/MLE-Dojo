...
def prepare(raw: Path, public: Path, private: Path):
    ...
    answers_df.to_csv(private / "test_answer.csv", index=False)
    ...
    random_predictions_df.to_csv(public / "sample_submission.csv", index=False)
    assert len(random_predictions_df) == len(
        test_ids
    ), f"Expected {len(test_ids)} samples in public/sample_submission.csv, but got {len(random_predictions_df)}"

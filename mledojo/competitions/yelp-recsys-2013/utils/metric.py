...
class YelpRecsys2013Metrics(CompetitionMetrics):
    """Metric class for the Yelp Recsys 2013 competition using Root Mean Squared Error (RMSE)"""
    def __init__(self, value: str = "Stars", higher_is_better: bool = False):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert the first column (RecommendationId) to string type for both dataframes
        y_true[y_true.columns[0]] = y_true[y_true.columns[0]].astype(str)
        y_pred[y_pred.columns[0]] = y_pred[y_pred.columns[0]].astype(str)
        
        # Sort both dataframes by the first column (RecommendationId) to ensure correct order
        y_true = y_true.sort_values(by=y_true.columns[0]).reset_index(drop=True)
        y_pred = y_pred.sort_values(by=y_pred.columns[0]).reset_index(drop=True)
        
        # Calculate RMSE between the true and predicted ratings
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Convert first column (RecommendationId) to string type
        submission[submission.columns[0]] = submission[submission.columns[0]].astype(str)
        ground_truth[ground_truth.columns[0]] = ground_truth[ground_truth.columns[0]].astype(str)
        
        # Sort the submission and ground truth by the first column (RecommendationId)

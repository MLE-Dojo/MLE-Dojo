...
class AllstateClaimsSeverityMetrics(CompetitionMetrics):
    """Metric class for Allstate Claims Severity competition using Mean Absolute Error (MAE)"""
    ...
    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        ...
        # Calculate MAE between the true and predicted loss values
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        if (submission[submission.columns[0]].values != ground_truth[ground_truth.columns[0]].values).any():
            raise InvalidSubmissionError("ID column values do not match between submission and ground truth. Please ensure the IDs are identical and in the same order.")

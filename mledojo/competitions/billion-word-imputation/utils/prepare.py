...
from mledojo.competitions.utils import extract, get_logger
...
def prepare(raw: Path, public: Path, private: Path):
    ...
    shutil.copy(public / "test_v2.txt", public / "sample_submission.csv")


    # Checks
    assert (public / "train_v2.txt").exists(), "public / 'train_v2.txt' should exist"
    assert (public / "test_v2.txt").exists(), "public / 'test_v2.txt' should exist"
    ...
    os.rename(private / "test.csv", private / "test_answer.csv")
    
    assert (
        count_lines_in_file(public / "sample_submission.csv") == private_test_line_count
    ), "public / 'sample_submission.csv' has incorrect number of lines"
    assert (

...
class DemandForecastingKernelsOnlyMetrics(CompetitionMetrics):
    """Metric class for Demand Forecasting Kernels Only competition using SMAPE"""
    def __init__(self, value: str = "sales", higher_is_better: bool = False):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert id column (first column) to string type and sort by it
        ...
        return (
            ...
            * np.sum(
                2
                * np.abs(y_pred[self.value] - y_true[self.value])
                / (np.abs(y_true[self.value]) + np.abs(y_pred[self.value]))
            )
        )

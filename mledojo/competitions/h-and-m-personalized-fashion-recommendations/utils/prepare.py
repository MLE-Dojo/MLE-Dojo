...
def prepare(raw: Path, public: Path, private: Path):
    ...
    # )
    # Create sample submission with all customer IDs
    # Get unique customer IDs from the training data
    all_customers = new_train["customer_id"].unique()
    
    # Create a sample submission DataFrame with the same columns as answers
    sample_submission = pd.DataFrame({
        "customer_id": answers["customer_id"],
        "prediction": ""  # Empty prediction column
    })
    
    # Write sample submission to CSV
    sample_submission.to_csv(public / "sample_submission.csv", index=False)
    
    # Checks for sample submission
    assert sample_submission.columns.tolist() == answers.columns.tolist(), "Sample submission and answers should have the same columns"
    assert sample_submission["customer_id"].nunique() == len(sample_submission), "There should be no duplicate customer_ids in sample submission"
    # Write CSVs

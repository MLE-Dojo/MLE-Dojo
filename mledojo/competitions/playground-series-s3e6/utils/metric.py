...
class PlaygroundSeriesS3E6Metrics(CompetitionMetrics):
    """Metric class for Playground Series S3E6 competition using Root Mean Squared Error (RMSE)"""
    ...
    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        ...
        # Calculate RMSE
        rmse = np.sqrt(mean_squared_error(y_true[self.value], y_pred[self.value]))
        return rmse
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Check if first columns (ids) are identical

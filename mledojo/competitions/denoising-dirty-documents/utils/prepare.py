...
from mledojo.competitions.utils import extract
...
def prepare(raw: Path, public: Path, private: Path) -> None:
    ...
    answers.to_csv(private / "test_answer.csv", index=False)
    ...
    assert read_csv(private / "test_answer.csv").drop_duplicates().shape[0] == len(
        read_csv(private / "test_answer.csv")
    ), (
        f"Expected `test_answer.csv` to have unique rows, but got "
        f"{read_csv(private / 'test_answer.csv').drop_duplicates().shape[0]} unique rows and "
        f"{len(read_csv(private / 'test_answer.csv'))} rows in total."
    )

    assert len(read_csv(private / "test_answer.csv")) == len(
        read_csv(public / "sample_submission.csv")
    ), (
        f"Expected `test_answer.csv` and `sample_submission.csv` to have the same number of rows, but "
        f"got {len(read_csv(private / 'test_answer.csv'))} rows in `test_answer.csv` and "
        f"{len(read_csv(public / 'sample_submission.csv'))} rows in `sample_submission.csv`."
    )

    assert "id" in read_csv(private / "test_answer.csv").columns, (
        f"Expected `test_answer.csv` to have an 'id' column, but got "
        f"{read_csv(private / 'test_answer.csv').columns}."
    )

    assert "value" in read_csv(private / "test_answer.csv").columns, (
        f"Expected `test_answer.csv` to have a 'value' column, but got "
        f"{read_csv(private / 'test_answer.csv').columns}."
    )

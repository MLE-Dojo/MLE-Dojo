...
def prepare(raw: Path, public: Path, private: Path):
    ...
    # sort sample_submission by the first column
    sample_submission = sample_submission.sort_values(by=sample_submission.columns[0])
    sample_submission.to_csv(public / "sample_submission.csv", index=False)
    answers.to_csv(private / "answers.csv", index=False)
    gold.to_csv(private / "test_answer.csv", index=False)

...
class PlaygroundSeriesS3e2Metrics(CompetitionMetrics):
    """Metric class for Playground Series S3E2 competition using ROC AUC"""
    def __init__(self, value: str = "stroke", higher_is_better: bool = True):
    ...
    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        ...
        # Sort dataframes by the id column to ensure alignment
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        if (submission[submission.columns[0]].values != ground_truth[ground_truth.columns[0]].values).any():
            raise InvalidSubmissionError("ID values do not match between submission and ground truth. Please ensure the ID column values are identical.")

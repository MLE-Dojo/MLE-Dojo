...
class CatInTheDatMetrics(CompetitionMetrics):
    """Metric class for Cat in the Dat competition using ROC AUC"""
    def __init__(self, value: str = "target", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert 'id' column to string type to ensure proper sorting
        ...
        # Compute ROC AUC score between the true target values and predicted probabilities
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Convert 'id' column to string type for both submission and ground truth
        ...
        if not (submission["id"].values == ground_truth["id"].values).all():
            raise InvalidSubmissionError("The id values in the submission do not match those in the ground truth. Please ensure the 'id' column values are identical.")

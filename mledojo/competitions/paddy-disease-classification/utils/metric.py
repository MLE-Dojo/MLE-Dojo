...
class PaddyDiseaseClassificationMetrics(CompetitionMetrics):
    """Metric class for Paddy Disease Classification competition using Accuracy Score"""
    def __init__(self, value: str = "label", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert first column (image_id) to string type
        y_true[y_true.columns[0]] = y_true[y_true.columns[0]].astype(str)
        y_pred[y_pred.columns[0]] = y_pred[y_pred.columns[0]].astype(str)
        # Sort the dataframes by the id column (image_id)
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        if not (submission[submission.columns[0]].values == ground_truth[ground_truth.columns[0]].values).all():
            raise InvalidSubmissionError("First column values (image_id) do not match between submission and ground truth. Please ensure the first column values are identical.")

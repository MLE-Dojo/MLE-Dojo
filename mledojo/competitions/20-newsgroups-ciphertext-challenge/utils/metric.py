...
class TwentyNewsgroupsCiphertextChallengeMetrics(CompetitionMetrics):
    ...
    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        ...
        return f1_score(y_true[self.value], y_pred[self.value], average="macro")
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Convert the ID columns (first column) to string type and sort by them
        ...
        if (submission[submission.columns[0]].values != ground_truth[ground_truth.columns[0]].values).any():
            raise InvalidSubmissionError("ID values do not match between submission and ground truth. Please ensure they are identical.")

...
class PokerRuleInductionMetrics(CompetitionMetrics):
    """Metric class for the Poker Rule Induction competition using categorization accuracy"""
    def __init__(self, value: str = "hand", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Ensure the id columns (first column) are strings for both DataFrames
        y_true[y_true.columns[0]] = y_true[y_true.columns[0]].astype(str)
        y_pred[y_pred.columns[0]] = y_pred[y_pred.columns[0]].astype(str)
        # Sort both DataFrames by the id column (first column)
        y_true = y_true.sort_values(by=y_true.columns[0]).reset_index(drop=True)
        y_pred = y_pred.sort_values(by=y_pred.columns[0]).reset_index(drop=True)
        # Calculate categorization accuracy using the 'hand' column

...
class PlaygroundSeriesS3E11Metrics(CompetitionMetrics):
    """Metric class for playground-series-s3e11 competition using RMSLE"""
    def __init__(self, value: str = "cost", higher_is_better: bool = False):
    ...
    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        ...
        # Calculate RMSLE using sklearn's mean_squared_log_error and then take the square root
        score = np.sqrt(mean_squared_log_error(y_true[self.value], y_pred[self.value]))
        return score

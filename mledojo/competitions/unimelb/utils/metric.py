...
class UnimelbMetrics(CompetitionMetrics):
    """Metric class for the University of Melbourne research grant success prediction competition using ROC AUC"""
    def __init__(self, value: str = "Grant.Status", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert first column (ID) to string type and sort by this column
        y_true[y_true.columns[0]] = y_true[y_true.columns[0]].astype(str)
        y_pred[y_pred.columns[0]] = y_pred[y_pred.columns[0]].astype(str)
        y_true = y_true.sort_values(by=y_true.columns[0]).reset_index(drop=True)
        y_pred = y_pred.sort_values(by=y_pred.columns[0]).reset_index(drop=True)
        
        # Compute ROC AUC: y_true[self.value] is the true Grant Status (0 or 1)
        # y_pred[self.value] is the predicted probability for success

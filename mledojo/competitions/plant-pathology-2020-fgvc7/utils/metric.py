...
class PlantPathology2020Fgvc7Metrics(CompetitionMetrics):
    """Metric class for Plant Pathology 2020 FGVC7 competition using mean column-wise ROC AUC"""
    ...
    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        ...
        for column in y_true.columns:
            if column != "image_id":
                try:
                    auc = roc_auc_score(y_true[column], y_pred[column])
                    aucs.append(auc)
                except ValueError:

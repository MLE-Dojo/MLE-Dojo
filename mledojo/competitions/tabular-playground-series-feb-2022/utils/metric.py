...
class TabularPlaygroundSeriesFeb2022Metrics(CompetitionMetrics):
    """Metric class for Tabular Playground Series Feb 2022 competition using classification accuracy."""
    def __init__(self, value: str = "target", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert first column (row_id) to string type
        y_true[y_true.columns[0]] = y_true[y_true.columns[0]].astype(str)
        y_pred[y_pred.columns[0]] = y_pred[y_pred.columns[0]].astype(str)
        # Sort by the first column "row_id"

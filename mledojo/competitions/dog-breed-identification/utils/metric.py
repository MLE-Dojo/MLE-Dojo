...
class DogBreedIdentificationMetrics(CompetitionMetrics):
    """Metric class for Dog Breed Identification competition using Multi Class Log Loss"""
    ...
    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        ...
        epsilon = 1e-15
        y_pred = np.clip(y_pred.iloc[:, 1:], epsilon, 1 - epsilon)
        return -(y_true.iloc[:, 1:] * np.log(y_pred)).sum(axis=1).mean()

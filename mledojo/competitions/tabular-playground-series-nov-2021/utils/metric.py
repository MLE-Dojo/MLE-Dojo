...
class TabularPlaygroundSeriesNov2021Metrics(CompetitionMetrics):
    """
    Metric class for Tabular Playground Series Nov 2021 competition,
    evaluated on area under the ROC curve of the predicted probabilities.
    """
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Check if the id columns in submission and ground truth are identical
        if not (submission[submission.columns[0]].values == ground_truth[ground_truth.columns[0]].values).all():
            raise InvalidSubmissionError("The id columns do not match between submission and ground truth. Please ensure the id columns are identical and in the same order.")

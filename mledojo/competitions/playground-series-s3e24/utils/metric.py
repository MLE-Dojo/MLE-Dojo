...
class PlaygroundSeriesS3e24Metrics(CompetitionMetrics):
    """Metric class for playground-series-s3e24 competition using area under the ROC curve (AUC-ROC)."""
    def __init__(self, value: str = "smoking", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert first column (id) to string type for proper sorting
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        missing_cols = true_cols - submission_cols
        extra_cols = submission_cols - true_cols

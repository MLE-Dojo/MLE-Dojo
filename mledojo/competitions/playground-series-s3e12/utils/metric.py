...
class PlaygroundSeriesS3e12Metrics(CompetitionMetrics):
    def __init__(self, value: str = "target", higher_is_better: bool = True):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert first column (assumed to be the id) to string
        ...
        # Compute ROC AUC score between the true and predicted target values
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Convert the id column (first column) in both submission and ground truth to string type

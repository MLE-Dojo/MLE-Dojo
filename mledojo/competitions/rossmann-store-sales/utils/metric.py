...
class RossmannStoreSalesMetrics(CompetitionMetrics):
    """Metric class for Rossmann Store Sales competition using RMSPE."""
    def __init__(self, value: str = "Sales", higher_is_better: bool = False):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Ensure the first column (Id) is of string type for consistent sorting
        ...
        # Filter out rows with zero sales in ground truth, as they are ignored in scoring
        valid_mask = y_true[self.value] != 0
        if valid_mask.sum() == 0:
            raise ValueError("No valid rows with non-zero sales found in ground truth.")

        y_true_valid = y_true.loc[valid_mask]
        y_pred_valid = y_pred.loc[valid_mask]

        # Calculate the RMSPE
        percentage_errors = ((y_true_valid[self.value] - y_pred_valid[self.value]) / y_true_valid[self.value]) ** 2
        rmspe = np.sqrt(percentage_errors.mean())
        return rmspe

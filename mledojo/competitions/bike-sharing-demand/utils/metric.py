...
class BikeSharingDemandMetrics(CompetitionMetrics):
    """Metric class for Bike Sharing Demand competition using Root Mean Squared Logarithmic Error (RMSLE)"""
    def __init__(self, value: str = "count", higher_is_better: bool = False):
        super().__init__(higher_is_better)
        self.value = value

    def evaluate(self, y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
        # Convert the id column (datetime) to string type
        y_true[y_true.columns[0]] = y_true[y_true.columns[0]].astype(str)
        y_pred[y_pred.columns[0]] = y_pred[y_pred.columns[0]].astype(str)
        # Sort by the id column, which is 'datetime'
    ...
    def validate_submission(self, submission: Any, ground_truth: Any) -> str:
        ...
        # Convert the first column (datetime) to string type
        submission[submission.columns[0]] = submission[submission.columns[0]].astype(str)
        ground_truth[ground_truth.columns[0]] = ground_truth[ground_truth.columns[0]].astype(str)
        # Sort the submission and ground truth by the first column (datetime)
        ...
        # Check if first columns (datetime) are identical
